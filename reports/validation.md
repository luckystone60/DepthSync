...
|---|---:|---:|---:|---:|---:|---:|---:|
| 01 | 0.1734 → 0.0694 | 0.0660 → 0.0249 | 0.2308 → 0.0919 | 0.0044 → 0.0033 | 0.0039 → 0.0034 | 0.0012（54） | 7.161 |
| 02 | 0.0275 → 0.0296 | 0.0246 → 0.0225 | 0.0346 → 0.0395 | 0.0031 → 0.0022 | 0.0052 → 0.0039 | 0.0021（57） | 2.468 |
| 03 | 0.0463 → 0.0437 | 0.0338 → 0.0208 | 0.0546 → 0.0447 | 0.0041 → 0.0038 | 0.0035 → 0.0034 | 0.0003（47） | 2.455 |
...
- V1 墙面中位值全片范围：0.0321。
- V4 墙面中位值全片范围：0.0080；相对 DepthPro 中位值偏差：0.0024。
- 区域标签 Laplacian P95：0.000000。
- 墙边锚帧 MAE/P95：0.011242 / 0.023149；连续回退带中位宽度：46.0 px。
