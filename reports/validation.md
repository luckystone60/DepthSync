...
- V4.1 撤下运行时空间区域修正：固定标签的全局平移会在 01 第 0 帧产生左右竖缝，常量区域 offset 会在第 60 帧形成“7”形硬轮廓。默认路径改为自适应选择 8/16/32/64 有效节点、统一序列化为 64 节点的全局单调 LUT，从机制上不再产生空间接缝。
...
- V1 是逐帧全局 affine；V4.1 在锚帧自适应选择 8/16/32/64 有效节点的单调 LUT，随后统一为 64 节点定长结构，整段保持全局单调映射。
