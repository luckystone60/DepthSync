...
|---|---:|---:|---:|---:|---:|---:|---:|
| 01 | 0.1734 → 0.0125 | 0.0660 → 0.0296 | 0.2308 → 0.0262 | 0.0044 → 0.0022 | 0.0039 → 0.0037 | 0.0005（54） | 3.688 |
| 02 | 0.0275 → 0.0231 | 0.0246 → 0.0225 | 0.0346 → 0.0327 | 0.0031 → 0.0020 | 0.0052 → 0.0039 | 0.0019（57） | 2.970 |
| 03 | 0.0463 → 0.0437 | 0.0338 → 0.0208 | 0.0546 → 0.0447 | 0.0041 → 0.0038 | 0.0035 → 0.0034 | 0.0003（47） | 3.101 |
...
- V1 墙面中位值全片范围：0.0321。
- V3.7 墙面中位值全片范围：0.0002；相对 DepthPro 中位值偏差：0.0000。
- 墙面静态权重 Laplacian P95：0.000000（越低表示网格分层越弱）。
- 墙边锚帧 MAE/P95：0.000447 / 0.000534；连续回退带中位宽度：3.0 px。
