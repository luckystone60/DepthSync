...
|---|---:|---:|---:|---:|---:|---:|
| 01 | 0.1734 → 0.0009 | 0.0660 → 0.0268 | 0.2308 → 0.0018 | 0.0044 → 0.0009 | -0.0008（68） | 3.808 |
| 02 | 0.0275 → 0.0009 | 0.0246 → 0.0151 | 0.0346 → 0.0025 | 0.0031 → 0.0002 | -0.0003（57） | 3.534 |
| 03 | 0.0463 → 0.0145 | 0.0338 → 0.0103 | 0.0546 → 0.0236 | 0.0041 → 0.0044 | 0.0021（53） | 3.479 |
...
- V1 墙面中位值全片范围：0.0321。
- V3.6 墙面中位值全片范围：0.0000；相对 DepthPro 中位值偏差：0.0000。
- 墙面静态权重 Laplacian P95：0.000000（越低表示网格分层越弱）。
- 墙边锚帧 MAE/P95：0.000356 / 0.000352；连续回退带中位宽度：3.0 px。
