...
当前 V3.5 面向端侧实现：使用锚帧确定的 8 节点单调 LUT，每帧只更新两个受限修正参数；动态区域在照片锚点附近叠加一个 16×9 低分辨率残差网格，静态背景使用 64×36 掩码和一份 128×72 照片目标层。播放阶段不运行深度模型、不计算稠密光流。

端侧处理流程、中间数据规格、参数包、内存/计算量和回退策略见 [`docs/depthsync-v35-design.md`](docs/depthsync-v35-design.md)；可编辑流程图见 [`docs/depthsync-flow.drawio`](docs/depthsync-flow.drawio)。
...
8. 结合 block MV、时序深度 MAD 和 5%–95% 时序范围，生成 64×36 静态背景软掩码；深度边缘处腐蚀掩码，避免静态照片层侵入人物轮廓和后段新出现的头发区域。
9. 对静态权重执行一次 5×5 闭运算和低通正则，填平 block MV/置信度留下的孤立小孔；随后重新施加动态占用与照片深度边缘门控，避免平面墙出现规则网格分层。
10. 静态照片目标层使用 128×72，降低大平面上的分段插值；8-bit 对比视频使用固定亚 LSB 抖动抑制可视化伪轮廓，浮点同步深度不添加噪声。
11. 播放阶段用一次 7×7 深度形态学梯度生成边缘保护权重；16×9 残差和静态照片层都只在非边缘区域生效，避免粗网格在前后景之间产生渐变光晕。
12. 计算动态结果 `D_dyn = LUT(D) + W_edge·upsample(residual_grid)`，再做静态软融合 `D' = (1-M·W_edge)D_dyn + M·W_edge·D_photo_grid`。
