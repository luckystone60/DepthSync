...
当前 V3.1 面向端侧实现：使用锚帧确定的 8 节点单调 LUT，每帧只更新两个受限修正参数，并在照片锚点附近叠加一个 16×9 的低分辨率残差网格。播放阶段不运行深度模型、不计算稠密光流。
...
7. 16×9 残差网格复用低强度 block MV 传播，并在锚点前后 30 帧内缓慢衰减。
8. 播放阶段计算 `D' = LUT(D) + upsample(residual_grid)`。
...
`offline_prepare` 输出同步深度、逐帧 LUT、LUT 的小幅 affine 修正、残差网格、置信度和回退原因。生产环境只需保存参数表，播放时调用 `apply_frame`。
