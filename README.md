...
当前 V4.1 面向端侧实现：照片深度只监督全局单调映射，不使用运行时空间掩码、区域标签、照片残差图或照片目标层。锚帧在 8/16/32/64 个有效节点中自适应选择，逐帧使用受门控的 16 节点分布稳定（强闪烁时为 64 节点），最终统一序列化为每帧 64 节点 LUT。播放阶段不运行深度模型、不需要人物分割 mask，也不计算光流。
...
4. 分别拟合 8、16、32、64 个有效节点的单调 LUT，并与鲁棒 affine 进行质量门控竞争。
5. 空间配对不可靠时采用无对应分位映射，避免静默回退 identity；最终统一为固定 64 节点。
6. 逐帧根据锚帧相关性和原始 VDA 分布跳变决定是否稳定值域；普通场景用 16 个控制点且限幅，强闪烁场景使用 64 点完整校正。
7. 锚点附近渐进解锁逐帧修正，避免锚帧与相邻帧参数状态跳变。
8. 所有像素使用同一条单调映射；不传播任何空间参数，因此不会产生区域边缘缝隙、轮廓或网格。

V4.1 默认不依赖端侧 ISP MV 或光流；评估工具中的稀疏运动仅用于计算运动补偿时序指标。
...
`offline_prepare` 输出同步深度、逐帧 64 节点 LUT、置信度和回退原因。FP16 参数包约 23 KiB/90 帧，播放时只读取当前帧约 256 B 的 LUT 并调用 `apply_frame`。
