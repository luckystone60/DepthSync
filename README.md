...
当前 V3.4 面向端侧实现：使用锚帧确定的 8 节点单调 LUT，每帧只更新两个受限修正参数；动态区域在照片锚点附近叠加一个 16×9 低分辨率残差网格，静态背景复用一份 64×36 的照片目标层。播放阶段不运行深度模型、不计算稠密光流。
...
10. 播放阶段用一次 7×7 深度形态学梯度生成边缘保护权重；16×9 残差和静态照片层都只在非边缘区域生效，避免粗网格在前后景之间产生渐变光晕。
11. 计算动态结果 `D_dyn = LUT(D) + W_edge·upsample(residual_grid)`，再做静态软融合 `D' = (1-M·W_edge)D_dyn + M·W_edge·D_photo_grid`。
