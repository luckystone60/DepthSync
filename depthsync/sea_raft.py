...
class SeaRaftEstimator:
    ...
    def __init__(
        ...
        self._backend: FlowBackend = _OfficialSeaRaftBackend(
            Path(checkpoint),
            Path(repository),
            device,
        )
    ...
    def estimate_pairs(
        ...
        if len(first_shape) != 2 or min(first_shape) <= 0:
            raise ValueError("frames must have non-empty image shapes")
        ...
        for frame in frames:
            ...
            if image.ndim != 3 or image.shape[2] != 3:
                raise ValueError("frames must be BGR uint8 images")
            resized = cv2.resize(
                image,
                (input_w, input_h),
        ...
        to_next: list[np.ndarray] = []
        to_previous: list[np.ndarray] = []
        confidence_next: list[np.ndarray] = []
        confidence_previous: list[np.ndarray] = []
        for index in range(len(prepared) - 1):
            with warnings.catch_warnings():
                ...
                forward, forward_log_variance = self._backend.predict(
                    prepared[index], prepared[index + 1]
                )
                backward, backward_log_variance = self._backend.predict(
                    prepared[index + 1], prepared[index]
                )
            ...
            forward_model_confidence = _variance_confidence(
                forward_log_variance,
                (input_h, input_w),
            )
            backward_model_confidence = _variance_confidence(
                backward_log_variance,
                (input_h, input_w),
            )
            ...
            confidence_previous.append(
                compute_flow_confidence(
                    backward,
                    forward,
                    backward_model_confidence,
                    gray[index + 1],
                    gray[index],
                )
            )
        sequence = DenseFlowSequence(
            to_next=np.stack(to_next),
            to_previous=np.stack(to_previous),
            confidence_next=np.stack(confidence_next),
            confidence_previous=np.stack(confidence_previous),
            scene_cuts=detect_scene_cuts(frames),
            frame_shape=(int(first_shape[0]), int(first_shape[1])),
        )
        return sequence.isolate_scene_cuts()
...
def _variance_confidence(
    ...
    value = np.asarray(log_variance, dtype=np.float32)
...
def mixture_log_variance(
    ...
    log_scales = np.empty_like(value[2:])
    log_scales[0] = np.clip(value[2], 0.0, var_max)
    log_scales[1] = np.clip(value[3], var_min, 0.0)
    return np.sum(weights * log_scales, axis=0).astype(np.float32)
...
def validate_checkpoint_keys(
    ...
    """Accept only BN buffers intentionally absent from the official file."""
    invalid_missing = [
        key
        for key in missing
        if not (
            key.startswith(("cnet.", "fnet."))
            and ".downsample.1." in key
        )
    ]
    if invalid_missing or unexpected:
        raise RuntimeError(
            "SEA-RAFT checkpoint mismatch: "
            f"missing={invalid_missing}, unexpected={list(unexpected)}"
        )


class _OfficialSeaRaftBackend:
    """Lazy boundary around the pinned upstream implementation."""
    ...
    def _predict_loaded(
        ...
        log_variance = mixture_log_variance(
            info,
            float(self._args.var_min),
            float(self._args.var_max),
        )
